...
exports.LunarUtil = void 0;
var Dictionary_1 = require("./Dictionary");
var LunarUtil = (function () {
    function LunarUtil() {
    }
    LunarUtil.getTimeZhiIndex = function (hm) {
        if (!hm) {
            return 0;
        }
        if (hm.length > 5) {
    ...
    LunarUtil.convertTime = function (hm) {
        return LunarUtil.ZHI[LunarUtil.getTimeZhiIndex(hm) + 1];
    };
    LunarUtil.getJiaZiIndex = function (ganZhi) {
        for (var i = 0, j = LunarUtil.JIA_ZI.length; i < j; i++) {
    ...
    LunarUtil.getDayYi = function (monthGanZhi, dayGanZhi) {
        var l = [];
        var day = LunarUtil.hex(LunarUtil.getJiaZiIndex(dayGanZhi));
        var month = LunarUtil.hex(LunarUtil.getJiaZiIndex(monthGanZhi));
        var right = LunarUtil.DAY_YI_JI;
        ...
        while (index > -1) {
            ...
            var i = void 0, m = void 0, j = void 0;
    ...
    LunarUtil.getDayJi = function (monthGanZhi, dayGanZhi) {
        var l = [];
        var day = LunarUtil.hex(LunarUtil.getJiaZiIndex(dayGanZhi));
        var month = LunarUtil.hex(LunarUtil.getJiaZiIndex(monthGanZhi));
        var right = LunarUtil.DAY_YI_JI;
    ...
    LunarUtil.getDayJiShen = function (lunarMonth, dayGanZhi) {
        var l = [];
        var day = LunarUtil.hex(LunarUtil.getJiaZiIndex(dayGanZhi));
        var month = Math.abs(lunarMonth).toString(16).toUpperCase();
        var index = LunarUtil.DAY_SHEN_SHA.indexOf(month + day + '=');
        if (index > -1) {
            var left = LunarUtil.DAY_SHEN_SHA.substr(index + 4);
            if (left.indexOf('=') > -1) {
    ...
    LunarUtil.getDayXiongSha = function (lunarMonth, dayGanZhi) {
        var l = [];
        var day = LunarUtil.hex(LunarUtil.getJiaZiIndex(dayGanZhi));
        var month = Math.abs(lunarMonth).toString(16).toUpperCase();
        var index = LunarUtil.DAY_SHEN_SHA.indexOf(month + day + '=');
    ...
    LunarUtil.getTimeYi = function (dayGanZhi, timeGanZhi) {
        var l = [];
        var day = LunarUtil.hex(LunarUtil.getJiaZiIndex(dayGanZhi));
        var time = LunarUtil.hex(LunarUtil.getJiaZiIndex(timeGanZhi));
        var index = LunarUtil.TIME_YI_JI.indexOf(day + time + '=');
        if (index > -1) {
            var left = LunarUtil.TIME_YI_JI.substr(index + 5);
            if (left.indexOf('=') > -1) {
    ...
    LunarUtil.getTimeJi = function (dayGanZhi, timeGanZhi) {
        var l = [];
        var day = LunarUtil.hex(LunarUtil.getJiaZiIndex(dayGanZhi));
        var time = LunarUtil.hex(LunarUtil.getJiaZiIndex(timeGanZhi));
        var index = LunarUtil.TIME_YI_JI.indexOf(day + time + '=');
    ...
    LunarUtil.getXunIndex = function (ganZhi) {
    ...
    LunarUtil.getXun = function (ganZhi) {
        return LunarUtil.XUN[LunarUtil.getXunIndex(ganZhi)];
    };
    LunarUtil.getXunKong = function (ganZhi) {
    ...
    LunarUtil.LIU_YAO = ['先胜', '友引', '先负', '佛灭', '大安', '赤口'];
    LunarUtil.HOU = ['初候', '二候', '三候'];
    LunarUtil.WU_HOU = ['蚯蚓结', '麋角解', '水泉动', '雁北乡', '鹊始巢', '雉始雊', '鸡始乳', '征鸟厉疾', '水泽腹坚', '东风解冻', '蛰虫始振', '鱼陟负冰', '獭祭鱼', '候雁北', '草木萌动', '桃始华', '仓庚鸣', '鹰化为鸠', '玄鸟至', '雷乃发声', '始电', '桐始华', '田鼠化为鴽', '虹始见', '萍始生', '鸣鸠拂奇羽', '戴胜降于桑', '蝼蝈鸣', '蚯蚓出', '王瓜生', '苦菜秀', '靡草死', '麦秋至', '螳螂生', '鵙始鸣', '反舌无声', '鹿角解', '蜩始鸣', '半夏生', '温风至', '蟋蟀居壁', '鹰始挚', '腐草为萤', '土润溽暑', '大雨行时', '凉风至', '白露降', '寒蝉鸣', '鹰乃祭鸟', '天地始肃', '禾乃登', '鸿雁来', '玄鸟归', '群鸟养羞', '雷始收声', '蛰虫坯户', '水始涸', '鸿雁来宾', '雀入大水为蛤', '菊有黄花', '豺乃祭兽', '草木黄落', '蛰虫咸俯', '水始冰', '地始冻', '雉入大水为蜃', '虹藏不见', '天气上升地气下降', '闭塞而成冬', '鹖鴠不鸣', '虎始交', '荔挺出'];
    LunarUtil.GAN = ['', '甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'];
    LunarUtil.POSITION_XI = ['', '艮', '乾', '坤', '离', '巽', '艮', '乾', '坤', '离', '巽'];
    LunarUtil.POSITION_YANG_GUI = ['', '坤', '坤', '兑', '乾', '艮', '坎', '离', '艮', '震', '巽'];
    LunarUtil.POSITION_YIN_GUI = ['', '艮', '坎', '乾', '兑', '坤', '坤', '艮', '离', '巽', '震'];
    LunarUtil.POSITION_FU = ['', '巽', '巽', '震', '震', '坎', '离', '坤', '坤', '乾', '兑'];
    LunarUtil.POSITION_FU_2 = ['', '坎', '坤', '乾', '巽', '艮', '坎', '坤', '乾', '巽', '艮'];
    LunarUtil.POSITION_CAI = ['', '艮', '艮', '坤', '坤', '坎', '坎', '震', '震', '离', '离'];
    LunarUtil.POSITION_TAI_SUI_YEAR = ['坎', '艮', '艮', '震', '巽', '巽', '离', '坤', '坤', '兑', '坎', '坎'];
    LunarUtil.POSITION_GAN = ['震', '震', '离', '离', '中', '中', '兑', '兑', '坎', '坎'];
    LunarUtil.POSITION_ZHI = ['坎', '中', '震', '震', '中', '离', '离', '中', '兑', '兑', '中', '坎'];
    LunarUtil.POSITION_TAI_DAY = ['占门碓 外东南', '碓磨厕 外东南', '厨灶炉 外正南', '仓库门 外正南', '房床栖 外正南', '占门床 外正南', '占碓磨 外正南', '厕灶厨 外西南', '仓库炉 外西南', '房床门 外西南', '门碓栖 外西南', '碓磨床 外西南', '厨灶碓 外西南', '仓库厕 外正西', '房床炉 外正西', '占大门 外正西', '碓磨栖 外正西', '厨房床 外正西', '仓库碓 外西北', '房床厕 外西北', '占门炉 外西北', '门碓磨 外西北', '厨灶栖 外西北', '仓库床 外西北', '房床碓 外正北', '占门厕 外正北', '碓磨炉 外正北', '厨灶门 外正北', '仓库栖 外正北', '占房床 房内北', '占门碓 房内北', '碓磨厕 房内北', '厨灶炉 房内北', '门仓库 房内北', '床房栖 房内中', '占门床 房内中', '占碓磨 房内南', '厨磨厕 房内南', '仓库炉 房内南', '房床门 房内西', '门碓栖 房内东', '碓磨床 房内东', '厨灶碓 房内东', '仓库厕 房内东', '房床炉 房内中', '占大门 外东北', '碓磨栖 外东北', '厨灶床 外东北', '仓库碓 外东北', '房床厕 外东北', '占门炉 外东北', '门碓磨 外正东', '厨灶栖 外正东', '仓库床 外正东', '房床碓 外正东', '占门厕 外正东', '碓磨炉 外东南', '厨灶门 外东南', '仓库栖 外东南', '占房床 外东南'];
    LunarUtil.POSITION_TAI_MONTH = ['占房床', '占户窗', '占门堂', '占厨灶', '占房床', '占床仓', '占碓磨', '占厕户', '占门房', '占房床', '占灶炉', '占房床'];
    LunarUtil.ZHI = ['', '子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'];
    LunarUtil.ZHI_XING = ['', '建', '除', '满', '平', '定', '执', '破', '危', '成', '收', '开', '闭'];
    LunarUtil.JIA_ZI = ['甲子', '乙丑', '丙寅', '丁卯', '戊辰', '己巳', '庚午', '辛未', '壬申', '癸酉', '甲戌', '乙亥', '丙子', '丁丑', '戊寅', '己卯', '庚辰', '辛巳', '壬午', '癸未', '甲申', '乙酉', '丙戌', '丁亥', '戊子', '己丑', '庚寅', '辛卯', '壬辰', '癸巳', '甲午', '乙未', '丙申', '丁酉', '戊戌', '己亥', '庚子', '辛丑', '壬寅', '癸卯', '甲辰', '乙巳', '丙午', '丁未', '戊申', '己酉', '庚戌', '辛亥', '壬子', '癸丑', '甲寅', '乙卯', '丙辰', '丁巳', '戊午', '己未', '庚申', '辛酉', '壬戌', '癸亥'];
    LunarUtil.TIAN_SHEN = ['', '青龙', '明堂', '天刑', '朱雀', '金匮', '天德', '白虎', '玉堂', '天牢', '玄武', '司命', '勾陈'];
    LunarUtil.ZHI_TIAN_SHEN_OFFSET = new Dictionary_1.Dictionary([
        ['子', 4],
        ['丑', 2],
        ['寅', 0],
        ['卯', 10],
    ...
    LunarUtil.TIAN_SHEN_TYPE = new Dictionary_1.Dictionary([
        ['青龙', '黄道'],
        ['明堂', '黄道'],
        ['金匮', '黄道'],
        ['天德', '黄道'],
    ...
    LunarUtil.TIAN_SHEN_TYPE_LUCK = new Dictionary_1.Dictionary([
        ['黄道', '吉'],
        ['黑道', '凶']
    ]);
    LunarUtil.LU = new Dictionary_1.Dictionary([
        ['甲', '寅'],
        ['乙', '卯'],
        ['丙', '巳'],
        ['丁', '午'],
    ...
    LunarUtil.PENGZU_GAN = ['', '甲不开仓财物耗散', '乙不栽植千株不长', '丙不修灶必见灾殃', '丁不剃头头必生疮', '戊不受田田主不祥', '己不破券二比并亡', '庚不经络织机虚张', '辛不合酱主人不尝', '壬不泱水更难提防', '癸不词讼理弱敌强'];
    LunarUtil.PENGZU_ZHI = ['', '子不问卜自惹祸殃', '丑不冠带主不还乡', '寅不祭祀神鬼不尝', '卯不穿井水泉不香', '辰不哭泣必主重丧', '巳不远行财物伏藏', '午不苫盖屋主更张', '未不服药毒气入肠', '申不安床鬼祟入房', '酉不会客醉坐颠狂', '戌不吃犬作怪上床', '亥不嫁娶不利新郎'];
    LunarUtil.NUMBER = ['〇', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二'];
    LunarUtil.MONTH = ['', '正', '二', '三', '四', '五', '六', '七', '八', '九', '十', '冬', '腊'];
    LunarUtil.SEASON = ['', '孟春', '仲春', '季春', '孟夏', '仲夏', '季夏', '孟秋', '仲秋', '季秋', '孟冬', '仲冬', '季冬'];
    LunarUtil.SHENGXIAO = ['', '鼠', '牛', '虎', '兔', '龙', '蛇', '马', '羊', '猴', '鸡', '狗', '猪'];
    LunarUtil.DAY = ['', '初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十', '十一', '十二', '十三', '十四', '十五', '十六', '十七', '十八', '十九', '二十', '廿一', '廿二', '廿三', '廿四', '廿五', '廿六', '廿七', '廿八', '廿九', '三十'];
    LunarUtil.YUE_XIANG = ['', '朔', '既朔', '蛾眉新', '蛾眉新', '蛾眉', '夕月', '上弦', '上弦', '九夜', '宵', '宵', '宵', '渐盈凸', '小望', '望', '既望', '立待', '居待', '寝待', '更待', '渐亏凸', '下弦', '下弦', '有明', '有明', '蛾眉残', '蛾眉残', '残', '晓', '晦'];
    LunarUtil.XIU = new Dictionary_1.Dictionary([
        ['申1', '毕'],
        ['申2', '翼'],
        ['申3', '箕'],
        ['申4', '奎'],
    ...
    LunarUtil.CHONG = ['午', '未', '申', '酉', '戌', '亥', '子', '丑', '寅', '卯', '辰', '巳'];
    LunarUtil.CHONG_GAN = ['戊', '己', '庚', '辛', '壬', '癸', '甲', '乙', '丙', '丁'];
    LunarUtil.CHONG_GAN_TIE = ['己', '戊', '辛', '庚', '癸', '壬', '乙', '甲', '丁', '丙'];
    LunarUtil.CHONG_GAN_4 = ['庚', '辛', '壬', '癸', '', '', '甲', '乙', '丙', '丁'];
    LunarUtil.HE_GAN_5 = ['己', '庚', '辛', '壬', '癸', '甲', '乙', '丙', '丁', '戊'];
    LunarUtil.HE_ZHI_6 = ['丑', '子', '亥', '戌', '酉', '申', '未', '午', '巳', '辰', '卯', '寅'];
    LunarUtil.SHA = new Dictionary_1.Dictionary([
        ['子', '南'], ['丑', '东'], ['寅', '北'], ['卯', '西'], ['辰', '南'], ['巳', '东'], ['午', '北'], ['未', '西'], ['申', '南'], ['酉', '东'], ['戌', '北'], ['亥', '西']
    ]);
    LunarUtil.POSITION_DESC = new Dictionary_1.Dictionary([
        ['坎', '正北'], ['艮', '东北'], ['震', '正东'], ['巽', '东南'], ['离', '正南'], ['坤', '西南'], ['兑', '正西'], ['乾', '西北'], ['中', '中宫']
    ]);
    LunarUtil.NAYIN = new Dictionary_1.Dictionary([
    ...
    LunarUtil.WU_XING_GAN = new Dictionary_1.Dictionary([
        ['甲', '木'], ['乙', '木'], ['丙', '火'], ['丁', '火'], ['戊', '土'], ['己', '土'], ['庚', '金'], ['辛', '金'], ['壬', '水'], ['癸', '水']
    ]);
    LunarUtil.WU_XING_ZHI = new Dictionary_1.Dictionary([
        ['寅', '木'], ['卯', '木'], ['巳', '火'], ['午', '火'], ['辰', '土'], ['丑', '土'], ['戌', '土'], ['未', '土'], ['申', '金'], ['酉', '金'], ['亥', '水'], ['子', '水']
    ]);
    LunarUtil.SHI_SHEN_GAN = new Dictionary_1.Dictionary([
        ['甲甲', '比肩'],
        ['甲乙', '劫财'],
        ['甲丙', '食神'],
        ['甲丁', '伤官'],
    ...
    LunarUtil.SHI_SHEN_ZHI = new Dictionary_1.Dictionary([
        ['甲子癸', '正印'],
        ['甲丑癸', '正印'],
        ['甲丑己', '正财'],
        ['甲丑辛', '正官'],
    ...
    LunarUtil.ZHI_HIDE_GAN = new Dictionary_1.Dictionary([
...
//# sourceMappingURL=LunarUtil.js.map