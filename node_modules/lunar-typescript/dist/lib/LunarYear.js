...
exports.LunarYear = void 0;
var LunarMonth_1 = require("./LunarMonth");
var Lunar_1 = require("./Lunar");
var ShouXingUtil_1 = require("./ShouXingUtil");
...
var LunarYear = (function () {
    ...
    LunarYear.prototype.compute = function () {
        ...
        for (i = 0, j = Lunar_1.Lunar.JIE_QI_IN_USE.length; i < j; i++) {
            var t = 36525 * ShouXingUtil_1.ShouXingUtil.saLonT((year + (17 + i) * 15.0 / 360) * ShouXingUtil_1.ShouXingUtil.PI_2);
            t += ShouXingUtil_1.ShouXingUtil.ONE_THIRD - ShouXingUtil_1.ShouXingUtil.dtT(t);
            this._jieQiJulianDays.push(t + Solar_1.Solar.J2000);
            if (i > 0 && i < 26) {
                jq[i - 1] = Math.round(t);
            }
        }
        var w = ShouXingUtil_1.ShouXingUtil.calcShuo(jq[0]);
        if (w > jq[0]) {
            w -= 29.5306;
        }
        for (i = 0; i < 16; i++) {
            hs.push(ShouXingUtil_1.ShouXingUtil.calcShuo(w + 29.5306 * i));
        }
        for (i = 0; i < 15; i++) {
        ...
        for (i = 0, j = dayCounts.length; i < j; i++) {
            ...
            this._months.push(new LunarMonth_1.LunarMonth(y, cm, dayCounts[i], hs[i] + Solar_1.Solar.J2000));
            if (!isNextLeap) {
                m++;
            }
            if (m == 13) {
    ...
    LunarYear.prototype._getZaoByGan = function (index, name) {
        ...
        var offset = index - Solar_1.Solar.fromJulianDay(month.getFirstJulianDay()).getLunar().getDayGanIndex();
        if (offset < 0) {
            offset += 10;
        }
        return name.replace('几', LunarUtil_1.LunarUtil.NUMBER[offset + 1]);
    };
    LunarYear.prototype._getZaoByZhi = function (index, name) {
        ...
        var offset = index - Solar_1.Solar.fromJulianDay(month.getFirstJulianDay()).getLunar().getDayZhiIndex();
        if (offset < 0) {
            offset += 12;
        }
        return name.replace('几', LunarUtil_1.LunarUtil.NUMBER[offset + 1]);
    ...
    LunarYear.prototype.getNineStar = function () {
        var index = LunarUtil_1.LunarUtil.getJiaZiIndex(this.getGanZhi()) + 1;
    ...
    LunarYear.prototype.getPositionTaiSui = function () {
        return LunarUtil_1.LunarUtil.POSITION_TAI_SUI_YEAR[this._zhiIndex];
    };
    ...
    LunarYear._LEAP = new Dictionary_1.Dictionary();
    LunarYear._CACHE = new Dictionary_1.Dictionary();
